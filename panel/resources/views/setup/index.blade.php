    ...
    <div class="flex items-center justify-center gap-2 mb-8">
        <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                 :class="step >= 1 ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-400'">1</div>
            <span class="text-sm" :class="step >= 1 ? 'text-white' : 'text-gray-500'">Admin</span>
        </div>
        <div class="w-8 h-px bg-gray-700"></div>
        <div class="flex items-center gap-2">
            <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold"
                 :class="step >= 2 ? 'bg-indigo-600 text-white' : 'bg-gray-700 text-gray-400'">2</div>
        ...
        <form method="POST" action="{{ route('setup.store') }}" novalidate @submit="loading = true">
            ...
            <div x-show="step === 1" x-cloak>
                ...
                <button type="button" @click="goNext()"
                        class="mt-4 w-full py-3 px-4 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg transition-colors">
                    Próximo
                </button>
            </div>
            ...
            <div x-show="step === 2" x-cloak>
                ...
                <div class="mt-6 flex gap-3">
                    ...
                    <button type="submit" :disabled="loading"
                            class="flex-1 py-3 px-4 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white font-semibold rounded-lg transition-colors flex items-center justify-center gap-2">
